...
class ISDeval:
    # Interface for evaluating detection on the Microsoft ISD dataset.
    #
    ...
    def __init__(self, isdGt=None, isdDt=None, isdAss = None,iouType='segm'):
        '''
        ...
        self.isdAss  = isdAss             # associations isd API
        self.evalImgs = defaultdict(list)   # per-image per-category evaluation results [KxAxI] elements
        self.eval     = {}                  # accumulated evaluation results
        self._gts = defaultdict(list)       # gt for evaluation
        self._dts = defaultdict(list)       # dt for evaluation
        self._ass_gts = defaultdict(list)       # gt for evaluation
        self._ass_dts = defaultdict(list)       # dt for evaluation
    ...
    def _prepare_ass(self):
        ...
        p = self.params
        if p.useCats:
            ass_gts=self.isdGt.loadAssoAnns(self.isdGt.getAssoAnnIds(imgIds=p.imgIds, catIds=1))
            ass_dts=self.isdAss.loadAssoAnns(self.isdAss.getAssoAnnIds(imgIds=p.imgIds, catIds=1))
        else:
            ass_gts=self.isdGt.loadAssoAnns(self.isdGt.getAssoAnnIds(imgIds=p.imgIds))
            ass_dts=self.isdDt.loadAssoAnns(self.isdDt.getAssoAnnIds(imgIds=p.imgIds))

        # convert ground truth to mask if iouType == 'segm'
        if p.iouType == 'segm':
            _toMask(ass_gts, self.isdGt)
            _toMask(ass_dts, self.isdAss)
        # set ignore flag
        for gt in ass_gts:
            gt['ignore'] = gt['ignore'] if 'ignore' in gt else 0
            gt['ignore'] = 'iscrowd' in gt and gt['iscrowd']
        ...
        self._ass_dts = defaultdict(list)       # dt for evaluation
        for gt in ass_gts:
            self._ass_gts[gt['image_id'], gt['category_id']].append(gt)
        for dt in ass_dts:
            self._ass_dts[dt['image_id'], dt['category_id']].append(dt)
        self.evalImgs = defaultdict(list)   # per-image per-category evaluation results
        self.eval     = {}                  # accumulated evaluation results
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        gt = self._ass_gts[imgId,1]
        dt = self._ass_dts[imgId,1]
        ...
        # print(ins_pred_class_ids)
        # print(instance_association)
